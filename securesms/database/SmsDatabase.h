...
public:
  ...
  public:
    Reader(OsIndependentCursor* cursor, OsIndependentContext* context);
...
public:
  virtual ~SmsDatabase();
  Reader* ReaderFor(OsIndependentCursor* cursor, OsIndependentContext* context) { return new Reader(cursor, context); }
...
public:
  ...
  Pair<long, long>* UpdateMessageBodyAndType(long messageId, unsigned char* body, long maskOff, long maskOn);
  Pair<long, long>* CopyMessageInbox(long messageId);
  Pair<long, long>* InsertMessageInbox(IncomingTextMessage* message, long type);
  Pair<long, long>* InsertMessageInbox(IncomingTextMessage* message);
...
const unsigned char* SmsDatabase::TAG = (unsigned char*)"SmsDatabase";

const unsigned char* SmsDatabase::TABLE_NAME = (unsigned char*)"sms";
const unsigned char* SmsDatabase::PERSON = (unsigned char*)"person";
const unsigned char* SmsDatabase::DATE_RECEIVED = (unsigned char*)"date";
const unsigned char* SmsDatabase::DATE_SENT = (unsigned char*)"date_sent";
const unsigned char* SmsDatabase::PROTOCOL = (unsigned char*)"protocol";
const unsigned char* SmsDatabase::STATUS = (unsigned char*)"status";
const unsigned char* SmsDatabase::TYPE = (unsigned char*)"type";
const unsigned char* SmsDatabase::REPLY_PATH_PRESENT = (unsigned char*)"reply_path_present";
const unsigned char* SmsDatabase::SUBJECT = (unsigned char*)"subject";
const unsigned char* SmsDatabase::SERVICE_CENTER = (unsigned char*)"service_center";
const unsigned char* SmsDatabase::CREATE_TABLE = (unsigned char*)"CREATE TABLE sms (_id integer PRIMARY KEY, thread_id INTEGER, address TEXT, address_device_id INTEGER DEFAULT 1, person INTEGER, date INTEGER, date_sent INTEGER, protocol INTEGER, read INTEGER DEFAULT 0, status INTEGER DEFAULT -1,type INTEGER, reply_path_present INTEGER, delivery_receipt_count INTEGER DEFAULT 0,subject TEXT, body TEXT, mismatched_identities TEXT DEFAULT NULL, service_center TEXT);";
const unsigned char* SmsDatabase::CREATE_INDEXS[] = {
  (unsigned char*)"CREATE INDEX IF NOT EXISTS sms_thread_id_index ON sms (thread_id);",
  (unsigned char*)"CREATE INDEX IF NOT EXISTS sms_read_index ON sms (read);",
  (unsigned char*)"CREATE INDEX IF NOT EXISTS sms_read_and_thread_id_index ON sms(read,thread_id);",
  (unsigned char*)"CREATE INDEX IF NOT EXISTS sms_type_index ON sms (type);",
  (unsigned char*)"CREATE INDEX IF NOT EXISTS sms_date_sent_index ON sms (date_sent);",
  (unsigned char*)"CREATE INDEX IF NOT EXISTS sms_thread_date_index ON sms (thread_id, date);"
};
const unsigned char* SmsDatabase::MESSAGE_PROJECTION[] = 
{
  ...
  (unsigned char*)"date AS date_received", (unsigned char*)"date_sent AS date_sent", 
  PROTOCOL, MmsSmsColumns::READ, STATUS, TYPE, REPLY_PATH_PRESENT, SUBJECT, 
